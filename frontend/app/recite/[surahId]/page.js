...
export default function RecitePage() {
  ...
  const [ayahIndex, setAyahIndex] = useState(0); // index into surah.ayahs
  ...
  async function handleStart() {
    ...
    try {
      const { session_id } = await api.startSession(
        surah.id,
        surah.ayahs[0].ayah_number,
        surah.ayahs[surah.ayahs.length - 1].ayah_number
      );
  ...
  function handleNextAyah() {
    reset();
    setWordResults(null);
    if (ayahIndex + 1 < surah.ayahs.length) {
    ...
    return (
      <>
        ...
        <main className="page">
          ...
          <p className="page-subtitle">{surah.ayah_count} ayahs · {surah.name_translation}</p>
          {!isSupported && (
    ...
    <>
      ...
      <main className="page">
        ...
        <p className="page-subtitle">
          Ayah {ayah.ayah_number} of {surah.ayah_count}
        </p>
        ...
        {!wordResults ? (
          ...
          <div className="card">
            ...
            <button onClick={handleNextAyah}>
              {ayahIndex + 1 < surah.ayahs.length ? "Next ayah" : "Finish session"}
            </button>
          </div>
        )}
      </main>
